...
class Growatt2MQTT:
    ...
    def init_growatt2mqtt(self):
        ...
        self.__interval = self.__settings.getint('query', 'interval', fallback=1)
        self.__offline_interval = self.__settings.getint('query', 'offline_interval', fallback=60)
        self.__error_interval = self.__settings.getint('query', 'error_interval', fallback=60)
        
        print('Setup Serial Connection... ', end='')
        self.__port = self.__settings.get('solarmon', 'port', fallback='/dev/ttyUSB0')
        self.__baudrate = self.__settings.get('solarmon', 'baudrate', fallback=9600)
